...
3. 华为的数据库，底层是用的Mongo，然后上层是架的那个ES加字节的一个一二级的一个缓存，然后相当于用户在就他们自己本地平台在使用的时候可以无感地去操作Mongo，然后的话并且能够使用到命中哈ES的一些特性和他们缓存的一些特性
4. 一些新型的数据库，比如说像卡萨多，然后这些这种高并发类型的数据库有没有去接触过呢
5. 真实的业务场景当中想去解决微服务架构里面的一些事务，如果说就对应着你目前的这种情况的话，你会去怎么样去设计这套架构
6. 有没有去了解过那个Mysql的一个一个多层架构呢？处理优化器，以及他的那个存储引擎这一块有没有去了解过呢？
7. 最近在使用和cloud code来进行那个项目开发，他们的区别又在哪里呢？MCP其实已经能够满足我们绝大多数的需求了，那为什么还需要skills这个东西？
8. 目前在项目当中，你目前是做什么样的一个工作呢？
9. 普罗米修斯的底层存储了解吗？
10. GRPC 一般是用于什么样的场景呢？
...
21. 缓存击穿的时候，一般情况下怎么处理？
    雪崩、键同时过期，
22. 你了解哪个消息队列？RMQ和Kafka都有什么区别？
23. 使用那个消息队列的时候，你如何去保证消息的可靠性，避免丢失？
24. 你能谈一下你的优缺点吗？
25. 你前端会吗？
