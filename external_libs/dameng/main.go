...
func demo2() {
	...
	sql := `INSERT INTO student (id, name, age, email) VALUES(3, 'John Doe', 30, 'john@example.com')`
	//_, err = db.Exec(sql)
	//if err != nil {
	//	return
	//}
	...
	for rows.Next() {
		result := student{}
		err = rows.Scan(&result.id, &result.name, &result.age, &result.email)
		if err != nil {
			continue
		}
...
var (
	driverName     = "dm"
	dataSourceName = "dm://SYSDBA:SYSDBA001@localhost:5236/dameng"
)

// SYSDBA:SYSDBA001, SYSDBA:SYSDBA
func demo1() {
	if db, err = connect(driverName, dataSourceName); err != nil {
		fmt.Println(err)
		return
